...
<td>短链（<code>b23.tv/...</code>）<br>视频av号（<code>www.bilibili.com/video/av...</code>）<br>视频BV号（<code>www.bilibili.com/video/BV...</code>）<br>消息中的独立av号 / BV号<br>番剧链接（<code>www.bilibili.com/bangumi/play/ep...</code>）<br>番剧链接（<code>www.bilibili.com/bangumi/play/ss...</code>）<br>动态长链（<code>www.bilibili.com/opus/...</code>）<br>动态短链（<code>t.bilibili.com/...</code>）<br>小程序卡片（<code>message.meta.detail_1.qqdocurl</code>）</td>
<td class="center">视频、图片、热评</td>
</tr>
<tr>
<td class="center"><strong>抖音 / TikTok</strong></td>
<td>抖音短链（<code>v.douyin.com/...</code>）<br>抖音视频长链（<code>www.douyin.com/video/...</code>）<br>抖音图集长链（<code>www.douyin.com/note/...</code>）<br>抖音作品分享链接（包含19位作品ID）<br>TikTok短链（<code>vm.tiktok.com/...</code>、<code>vt.tiktok.com/...</code>、<code>www.tiktok.com/t/...</code>）<br>TikTok视频/图集长链（<code>www.tiktok.com/@.../video/...</code>、<code>www.tiktok.com/@.../photo/...</code>）</td>
<td class="center">视频、图片</td>
</tr>
<tr>
<td class="center"><strong>快手</strong></td>
<td>短链（<code>v.kuaishou.com/...</code>）<br>快手长链（<code>www.kuaishou.com/...</code>）<br>分享页（<code>gifshow.com/...</code>、<code>chenzhongtech.com/...</code>）</td>
<td class="center">视频、图片</td>
</tr>
<tr>
...
<td>Web链接（<code>www.xiaoheihe.cn/app/topic/game/...</code>）<br>App分享链接（<code>api.xiaoheihe.cn/game/share_game_detail?...</code>）<br>BBS分享链接（<code>www.xiaoheihe.cn/app/bbs/link/...</code>、<code>www.xiaoheihe.cn/v3/bbs/...</code>）<br>Web分享接口（<code>api.xiaoheihe.cn/v3/bbs/app/api/web/share?...</code>）<br>小程序卡片（<code>message.meta.news.jumpUrl</code>）</td>
...
配置 **缓存目录** 可显著提升解析成功率和发送体验。当前版本没有“预下载所有媒体”开关，插件会按单个媒体自动选择直链发送、缓存后发送或跳过不可发送的媒体。
...
当 AstrBot 与消息平台协议端（如 NapCat、Lagrange）**不在同一台机器**或**无法共享文件目录**时，本地下载的媒体文件对协议端不可达。媒体中转模式通过 AstrBot 内置 HTTP 服务桥接，将已缓存的本地文件转为可回调的临时 URL 发送。
...
> **注意**：开启媒体中转后，不会强制下载所有媒体，也不会自动切换缓存目录；它只会增强已经成功缓存到本地的媒体文件。Token 注册失败时自动回退为本地文件发送。
...
- **抖音 / TikTok**：TikTok 页面解析会优先使用系统 `curl`，如目标环境没有 `curl` 会回退普通请求；TikTok 受地区和风控影响时建议开启代理
- **小红书**：链接有身份验证和时效性，部分分享链接解析结果有水印
- **小黑盒**：BBS 分享和部分视频解析依赖签名能力，需要安装 `cryptography`；游戏预览视频下载速度不佳时请启用代理
- **推特**：解析 api 优先使用 fxtwitter 服务，服务不可用时尝试 Guest GraphQL 回退；图片和视频 cdn 大多被墙建议开启代理
- **图片处理**：格式除 ```.jpg```, ```.jpeg```, ```.png``` 外的所有图片会先转换为 ```.png``` 格式再发送，转换需要系统安装 `ffmpeg`
- **视频合并**：B站 DASH、M3U8 音视频合并需要系统安装 `ffmpeg`
